...
public class ClientFileThread extends Thread {
    ...
    static PrintWriter out = null;  // 普通消息的发送（server.Server.java传来的值）
    ...
    public void run() {
        try {
            ...
            fileOut.writeUTF(userName);  //向服务器文件传输进程发送姓名
            fileOut.flush();
            // 接收文件
            while (true) {
                String textName = fileIn.readUTF();
                long totleLength = fileIn.readLong();
                String reminder = "Do you want to receive<" + textName + ">?";
                int result = JOptionPane.showConfirmDialog(chatViewJFrame, reminder, userName,
...
//					out.flush();
                    String userDir = "./userFile/";  //用户目录
                    File userFile = new File(userDir + userName);
                    if (!userFile.exists()) {  // 新建当前用户的文件夹
                        userFile.mkdirs();
                    }
                    File file = new File(userDir + userName + "\\" + textName);
...
//						out.flush();
                    ...
                    // 提示文件存放地址
                    JOptionPane.showMessageDialog(chatViewJFrame, "The file has been saved in:\n" +
                            userDir +
                            userName + "\\" + textName, userName, JOptionPane.INFORMATION_MESSAGE);
                } else {  // 不接受文件
    ...
    static void outFileToServer(String path) {
        try {
            ...
            out.println("PRIVATEMESSAGE " + userName + " " + "#我已成功发送文件！#");
            out.flush();
        } catch (Exception e) {
        }
    }
    ...
    // 客户端发送文件
    static void outFileToServer(String path, String friendName) {
        try {
            File file = new File(path);
            fileReader = new DataInputStream(new FileInputStream(file));
            fileOut.writeUTF(file.getName());  // 发送文件名字
            ...
            fileOut.writeLong(file.length());  // 发送文件长度
            fileOut.flush();
            fileOut.writeUTF(friendName);  // 接收方名字
            fileOut.flush();
            int length = -1;
            byte[] buff = new byte[1024];
            while ((length = fileReader.read(buff)) > 0) {  // 发送内容
            ...
            String msg = "PRIVATEMESSAGE " + friendName + " " + "I have sent the file successfully!";
            out.println(msg);
            out.flush();
        } catch (Exception e) {
        }
